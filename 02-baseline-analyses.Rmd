...
knitr::opts_chunk$set(warning = FALSE,
                      ...
                      fig.retina = 2,
                      fig.align = 'center',
                      fig.path = 'figures/02-baseline-analysis/',
                      fig.width = 7,
                      fig.height = 7)
```
...
Summary of key baseline demographic and study outcome measures.
...
map_df(.x = foo,
    ...
    arrange(`difference in location`) %>% 
    knitr::kable(caption = 'Difference in median BPI-PSS scores between study sites',
                 align = 'lrrr')
```
...
ggplot(data = bdi_score) +
    ...
    labs(title = 'BDI score by study site',
         subtitle = 'Dashed lines mark threshold scores for depression severity',
...
## Loop through the list, perform a Wilcox test and tabulate
for(i in 1:6) {
    foo[[i]] <- wilcox.test(x = bdi_score.ci[[pairs[1, i]]],
                          y = bdi_score.ci[[pairs[2, i]]],
...
knitr::kable(xtabs(~ Occupation_new + Study_site, 
                   data = occupation),
             caption = '(Simplified) employment status by study site (counts)')

knitr::kable(round(100 * prop.table(
    xtabs(~ Occupation_new + Study_site, 
          data = occupation)), 1),
...
knitr::kable(
    broom::tidy(
        fisher.test(xtabs(~Occupation_new + Study_site, data = occupation))),
...
knitr::kable(xtabs(~ SOS_mnemonic + Study_site, 
                   data = sos_lit),
             caption = 'Health literacy by study site (counts)')

knitr::kable(round(100 * prop.table(
    xtabs(~ SOS_mnemonic + Study_site, 
          data = sos_lit)), 1),
...
knitr::kable(
    broom::tidy(
        fisher.test(xtabs(~ SOS_mnemonic + Study_site, data = sos_lit))),
...
knitr::kable(xtabs(~ Years_education + Study_site, 
                   data = education),
             caption = 'Years of education by study site (counts)')

knitr::kable(round(100 * prop.table(
    xtabs(~ Years_education + Study_site, 
          data = education)), 1),
...
knitr::kable(
    broom::tidy(
        fisher.test(xtabs(~ Years_education + Study_site, data = education_new))),
