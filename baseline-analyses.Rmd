---
title: "Analysis of baseline data"
...
files <- list.files(path = './data/',
    ...
    .[!. %in% c('demographics.rds', 'simmonds.rds', 'bpi_factor_analysis.rds')] %>%
    # Add path
    paste0('./data/', .)
...
# Extract list to environment (what a cool function!)
list2env(data, 
         envir = .GlobalEnv)

# Select relevant baseline demographics variables
demographics %<>%
    select(ID, Site, Group, Age, CD4_recent, 
           SOS_mnemonic, Occupation, Years_education) %>%
    # Add a urban/rural code
...
# Define comparison pairs for site
pairs <- combn(x = c('r1', 'r2', 'u1', 'u2'), 
               m = 2) %>%
    t(.) %>%
    as_data_frame(.) %>%
    mutate(comparison = paste(V1, 'vs', V2))
```
...
ggplot(data = bpi_pss) +
    ...
    labs(title = 'PSS score by study site',
...
knitr::kable(
    broom::tidy(
        kruskal.test(PSS ~ as.factor(Site), 
                     data = bpi_pss)))
...
for(i in 1:6) {
    foo[[i]] <- wilcox.test(x = bpi_pss.ci[[pairs$V1[[i]]]],
                          y = bpi_pss.ci[[pairs$V2[[i]]]],
...
ggplot(data = bpi_pss) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    scale_y_continuous(breaks = seq(0, 10, 2)) +
    labs(title = 'PSS score by urban/rural',
         x = 'Study site',
         y = 'PSS')
...
knitr::kable(
    broom::tidy(
        wilcox.test(PSS ~ as.factor(Type), 
                    data = bpi_pss,
                    conf.int = TRUE)),
    caption = 'Median of the difference in BPI-PSS score (estimate) between urban and rural study sites')
```
...
ggplot(data = bpi_pis) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    scale_y_continuous(breaks = seq(0, 10, 2)) +
    labs(title = 'PIS score by study site',
         caption = '"r" = rural, "u" = urban',
         x = 'Study site',
         y = 'PIS')

# Hypothesis test: PIS by site
knitr::kable(
    broom::tidy(
        kruskal.test(PIS ~ as.factor(Site), 
                     data = bpi_pis)))
...
## Loop through the list, perform a Wilcox test and tabulate
for(i in 1:6) {
    foo[[i]] <- wilcox.test(x = bpi_pis.ci[[pairs$V1[[i]]]],
                          y = bpi_pis.ci[[pairs$V2[[i]]]],
                          conf.int = TRUE)[c(3, 8:9)]
...
ggplot(data = bpi_pis) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    scale_y_continuous(breaks = seq(0, 10, 2)) +
    labs(title = 'PIS score by urban/rural',
         x = 'Study site',
         y = 'PIS')
...
knitr::kable(
    broom::tidy(
        wilcox.test(PIS ~ as.factor(Type), 
                    data = bpi_pis,
                    conf.int = TRUE)),
    caption = 'Median of the difference in BPI-PIS score (estimate) between urban and rural study sites')
```
...
ggplot(data = se6_mean) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    scale_y_continuous(breaks = seq(0, 10, 2)) +
    labs(title = 'SE-6 score by study site',
         caption = '"r" = rural, "u" = urban',
         x = 'Study site',
         y = 'SE-6 score')
...
knitr::kable(
    broom::tidy(
        kruskal.test(Mean_score ~ as.factor(Site), 
                     data = se6_mean)))
...
## Loop through the list, perform a Wilcox test and tabulate
for(i in 1:6) {
    foo[[i]] <- wilcox.test(x = se6_mean.ci[[pairs$V1[[i]]]],
                          y = se6_mean.ci[[pairs$V2[[i]]]],
                          conf.int = TRUE)[c(3, 8:9)]
...
map_df(.x = foo,
    ...
    select(comparison, everything()) %>%
    arrange(p.value) %>%
    knitr::kable(caption = 'Median of the difference in SE-6 score (estimate) between study sites (pairwise Wilcoxon rank sum tests with Holm corrected p-value)')
...
ggplot(data = se6_mean) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    scale_y_continuous(breaks = seq(0, 10, 2)) +
    labs(title = 'SE-6 score by urban/rural',
         x = 'Study site',
         y = 'SE-6 score')
...
knitr::kable(
    broom::tidy(
        wilcox.test(Mean_score ~ as.factor(Type), 
                    data = se6_mean,
                    conf.int = TRUE)),
    caption = 'Median of the difference in SE-6 score (estimate) between urban and rural study sites')
```
...
eq5d_vas <- eq5d %>%
    # Select columns
    select(ID, Site, Type, SOH_BL) %>%
    # Rename column
    rename(VAS_score = SOH_BL)
```
...
ggplot(data = eq5d_vas) +
    ...
    geom_jitter(height = 0,
                shape = 21) +
    labs(title = 'EQ5D VAS score by study site',
         caption = '"r" = rural, "u" = urban',
         x = 'Study site',
         y = 'EQ5D VAS rating')

# Hypothesis test: EQ5D VAS by site
knitr::kable(
    broom::tidy(
        kruskal.test(VAS_score ~ as.factor(Site), 
                     data = eq5d_vas)))


# Delta confidence interval and p-value
## Simplify and spread the data
eq5d_vas.ci <- eq5d_vas %>%
    select(ID, Site, VAS_score) %>%
    spread(key = Site,
           value = VAS_score) 
...
## Loop through the list, perform a Wilcox test and tabulate
for(i in 1:6) {
    foo[[i]] <- wilcox.test(x = eq5d_vas.ci[[pairs$V1[[i]]]],
                          y = eq5d_vas.ci[[pairs$V2[[i]]]],
                          conf.int = TRUE)[c(3, 8:9)]
...
map_df(.x = foo,
    ...
    mutate(comparison = pairs$comparison) %>%
    select(comparison, everything()) %>%
    ### Remove p-value (not required)
    select(-p.value) %>%
    knitr::kable(caption = 'Median of the difference in EQ5D score (estimate) between study sites (pairwise Wilcoxon rank sum tests with Holm corrected p-value)')
...
# Hypothesis test: EQ5D VAS by urban/rural
knitr::kable(
    broom::tidy(
        wilcox.test(VAS_score ~ as.factor(Type), 
                    data = eq5d_vas,
                    conf.int = TRUE)),
    caption = 'Median of the difference in EQ5D VAS rating (estimate) between urban and rural study sites')
```
